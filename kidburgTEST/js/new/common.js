(function($){
  ...
  $('#scr_calendar .calendar__day').each(function() {
    if ( $(this).find('.bottom').length > 0 ) {
      $(this).addClass('calendar__day--with-event');
    }
  });

  /* Модалка "уже уходите?" */
  /* Кукисы - модалка не чаще 1 раз в неделю */
  ...
  if (alertwin != "no") { 
    ...
    $(document).mouseleave(function(e){
      if (e.clientY < 10) {
        if (leaveCount == 0) {
          $('#modal-leave').show();
          $('#modal-leave' + ' .modal__content').addClass('visible');
        }
        leaveCount = 1;
